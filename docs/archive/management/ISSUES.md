...
## 2026-04-08 复核结论（覆盖历史快照）

> 说明：本节用于覆盖旧结论，优先级高于下方历史记录。

### 当前状态摘要

1. `go test ./... -short`：✅ 通过
2. `go test -race ./internal/api`：✅ 通过
3. `go test ./internal/risk`：✅ 通过

### 本次闭环修复

1. 风控回归修复：`internal/risk/risk.go`
	- 减仓/反向单按净新增风险计算，避免被 `single_trade_risk` 与 `symbol_exposure` 误拦截。
2. API 并发竞态修复：`internal/api/server.go`、`internal/api/server_test.go`
	- `Start/Stop` 增加同步保护；
	- 新增 `IsRunning()` 受锁读取；
	- `TestStartStop` 改为受锁断言，`-race` 通过。

### 文档状态修订

1. 第四章“未实现功能”中以下条目已实现，需视为历史信息：
	- 条件单功能
	- 调整杠杆功能
	- 移动止损功能
	- 实时行情数据
	...
	- WebSocket 实时推送
	- 多渠道通知
2. API 密钥安全项仍为“部分完成”，历史提交是否泄漏密钥仍需人工策略确认与必要轮换。
...
> 注：本节为历史快照，已被本文顶部“2026-04-08 复核结论（覆盖历史快照）”覆盖。

| 功能 | 状态 | 说明 |
|------|------|------|
| 条件单功能 | ✅ 已实现 | 历史项，已在代码中实现 |
| 调整杠杆功能 | ✅ 已实现 | 历史项，已在代码中实现 |
| 移动止损功能 | ✅ 已实现 | 历史项，已在代码中实现 |
| 实时行情数据 | ✅ 已实现 | 历史项，已在代码中实现 |
| 技术指标计算 | ✅ 已实现 | 历史项，已在代码中实现 |
| WebSocket实时推送 | ✅ 已实现 | 历史项，已在代码中实现 |
| 多渠道通知 | ✅ 已实现 | 历史项，已在代码中实现 |
| 数据采集服务 | ⚠️ 部分实现 | 新闻/经济数据源仍有模拟数据路径 |
